...
class DeepFM(nn.Module):
    def __init__(self, cate_fea_nuniqs, nume_fea_size=0, emb_size=8,
                 hid_dims=[256, 128], num_classes=1, dropout=[0.05, 0.05]):
        """
    ...
    for _ in range(epochs):
        ...
        with torch.no_grad():
            ...
            for idx, x in tqdm(enumerate(valid_loader)):
                ...
                pred = model(cate_fea,nume_fea)
                val_loss = float(loss_fcn(pred,label))
                # pred = model(cate_fea, nume_fea).reshape(-1).data.cpu().numpy().tolist()
                # valid_preds.extend(pred)
                # valid_labels.extend(label.cpu().numpy().tolist())
        # cur_mse = mean_squared_error(valid_labels, valid_preds)
        if val_loss < best_loss:
            best_loss = val_loss
            torch.save(model.state_dict(), "data/deepfm_best.pth") # 日志回滚
...
if __name__ == '__main__':
    ...
    train_dataset = Data.TensorDataset(torch.LongTensor(train[sparse_features].values),
                                       ...
                                       torch.FloatTensor(np_utils.to_categorical(train['Severity'].values - 1, 4)) )
    ...
    # model = DeepFM(cate_fea_nuniqs, nume_fea_size=len(dense_features),emb_size=8,
    #              hid_dims=[256,128], num_classes=4, dropout=[0.05, 0.05])
    model = DeepFM(cate_fea_nuniqs, nume_fea_size=len(dense_features),emb_size=64,
                 hid_dims=[512,128], num_classes=4, dropout=[0.0, 0.0])

    # weights_path = 'data/deepfm_best.pth'
    # weights_dict = torch.load(weights_path, map_location='cpu')
    # model.load_state_dict(weights_dict) # 加载上一轮预训练的模型
    model.to(device)
    loss_fcn = nn.CrossEntropyLoss()  # Loss函数
    ...
    train_and_eval(model, train_loader, valid_loader, 50, device)